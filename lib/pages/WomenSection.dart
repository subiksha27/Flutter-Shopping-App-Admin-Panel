...
class WomenSection extends StatefulWidget {
  static const routeName = '/WomenSection';
  static String id = 'WomenSection';
...
class  _WomenSectionState extends State<WomenSection> {
  ...
  _WomenSectionState({this.auth, this.onSignedOut});
  final auth; //BaseAuth
  ...
  Future getPosts() async {
    ...
    QuerySnapshot qn = await firestore.collection("womensection").getDocuments();
    return qn.documents;
  }
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
        ...
        body: new StreamBuilder(
          stream: Firestore.instance.collection('womensection').snapshots(),
  ...
  _sendToServer(){
    if (_key.currentState.validate() ){
      ...
      Firestore.instance.runTransaction((Transaction transaction) async {
        CollectionReference reference = Firestore.instance.collection('womensection');
...
class _StatusState extends State<Pages>{
  Widget build(BuildContext context){
    return Scaffold(
        appBar:  AppBar(
          title: Text("Women Section  Details"),
          backgroundColor: Colors.blue[900],
          actions: <Widget>[
            IconButton(
              icon: Icon(
