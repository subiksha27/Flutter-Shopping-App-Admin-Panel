...
class KidsSection extends StatefulWidget {
  static const routeName = '/KidsSection';
  static String id = 'KidsSection';
  @override
  _KidsSectionState createState() => _KidsSectionState();
}
class  _KidsSectionState extends State<KidsSection> {
  _KidsSectionState({this.auth, this.onSignedOut});
  final auth; //BaseAuth
  ...
  Future getPosts() async {
    ...
    QuerySnapshot qn = await firestore.collection("kidssection").getDocuments();
    return qn.documents;
  }
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
        ...
        body: new StreamBuilder(
          stream: Firestore.instance.collection('kidssection').snapshots(),
  ...
  _sendToServer(){
    if (_key.currentState.validate() ){
      ...
      Firestore.instance.runTransaction((Transaction transaction) async {
        CollectionReference reference = Firestore.instance.collection('kidssection');
...
class _StatusState extends State<Pages>{
  Widget build(BuildContext context){
    return Scaffold(
        appBar:  AppBar(
          title: Text("Kids Section  Details"),
          backgroundColor: Colors.blue[900],
          actions: <Widget>[
            IconButton(
              icon: Icon(
