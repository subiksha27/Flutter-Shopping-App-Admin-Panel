...
class  _AccessoryState extends State<Accessory> {
  ...
  _AccessoryState({this.auth, this.onSignedOut});
  final auth; //BaseAuth
  ...
  Future getPosts() async {
    ...
    QuerySnapshot qn = await firestore.collection("accessory").getDocuments();
    return qn.documents;
  }
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
        ...
        body: new StreamBuilder(
          stream: Firestore.instance.collection('accessory').snapshots(),
                        ...
                        return ListView.builder(
                            ...
                            itemBuilder: (_, index) {
                              return ListTile(
                                ...
                                subtitle: new Text(snapshot.data[index].data["jewellery"]),
                                onTap: () =>
...
class _UploadFormFieldState extends State<UploadFormField> {
  ...
  String  topbrands,jewellery,quantity,pricerange,categories,others;
  ...
  Widget FormUI() {
    return new Column(
      children: <Widget>[
        ...
        new TextFormField(
            decoration: new InputDecoration(icon:Icon(Icons.add_shopping_cart),hintText: 'Jewellery'),
            cursorColor: Colors.pinkAccent,
            //validator: validateorder,
            onSaved: (String val) {
              jewellery = val;
  ...
  _sendToServer(){
    if (_key.currentState.validate() ){
      ...
      Firestore.instance.runTransaction((Transaction transaction) async {
        CollectionReference reference = Firestore.instance.collection('accessory');

        await reference.add({"topbrands":"$topbrands","jewellery": "$jewellery", "quantity": "$quantity","pricerange":"$pricerange","categories":"$categories","others":"$others"});
      });
    } else {
      // validation error
      setState(() {
...
class _StatusState extends State<Pages>{
  Widget build(BuildContext context){
    return Scaffold(
        appBar:  AppBar(
          title: Text("Accessories  Details"),
          backgroundColor: Colors.blue[900],
          actions: <Widget>[
            IconButton(
              icon: Icon(
                ...
                rows: [
                  DataRow(
                    cells: <DataCell>[
                      ...
                      DataCell(Text(widget.post.data['jewellery']),),
